...
class OpenMax(BasePostprocessor):
    ...
    def postprocess(self, net: nn.Module, data):
        ...
        for score in scores:
            ...
            # print("OpenMax prediction shape:", so.shape)
            pred_openmax.append(
                # np.argmax(so) if np.max(so) >= self.weibull_threshold else (self.nc - 1)
                # np.argmax(so) if np.max(so) >= self.weibull_threshold else -1
                np.argmax(so) if np.argmax(so) != self.nc else -1
            )
