...
def compute_all_metrics(conf, label, pred):
    ...
    id_a = acc(pred, label)
    ood_a = ood_acc(pred, label)
    all_a = all_acc(pred, label)
    print(pred, label, conf)

    results = [fpr, auroc, aupr_in, aupr_out, id_a, ood_a, all_a]
    # results = [fpr, auroc, aupr_in, aupr_out, accuracy]

    return results


def get_w_mse(c, pi_gt, pi):
    return np.mean(np.power(pi_gt / c - pi / c, 2))


# overall accuracy
def all_acc(pred, label):
    acc = np.sum(pred == label) / len(label)
    return acc


# ood accuracy
def ood_acc(pred, label):
    ood_pred = pred[label == -1]
    ood_label = label[label == -1]

    num_tp = np.sum(ood_pred == ood_label)
    acc = num_tp / len(ood_label)
