...
def imb_subset(dataset, imglist_path, cfg, random: bool = False):
    ...
    if cfg['mode'] == 'Original':
        label_set, cls_num_list = np.unique(labels, return_counts=True)
        subset = dataset
    else:
        print('Subsampling original dataset ...')
        subset_loc_list, cls_num_list = create_imb_subset_idx(labels, cfg=cfg,
                                                              return_count=True,
                                                              random=random)
        subset = Subset(dataset, subset_loc_list)
        print('Subsampling Finished.')

    return subset, list(cls_num_list)


# Imbalanced Test Set Generator-----------------------------------------------#
def create_imb_subset_idx(labels: List[int], order_idx: np.ndarray = None, cfg: dict = None,
    ...
    Args:
        ...
        random:             If use random random seed.
    ...
    """
    if cfg is None:
        print('cfg is None, return original labels.')
        return labels
    else:
        # print('Create subset based on ' + str(cfg))
        if random:
            np.random.seed()
        # pass
        ...
        print("label nums: %i, max: %i, min: %i" % (len(label_set), label_set.max(), label_set.min()))

        if len(sample_nums) == 1:
            # print('Uniform dataset, per-cls sample numbers counts is: %s' % str(sample_nums))
            pass
        else:
            print('[warning] Dataset is not uniform, with per-class sample num: %s, '
        ...
        if 'data_ratio' in cfg.keys():
            ...
            if cfg['data_ratio'] != 1:
                ...
                print('Reduce test set samples with ratio given by cfg["reduce"] = %.4f'
                             % cfg['data_ratio'])
                print('Original per-cls sample No. %i, Reduced per-cls sample No. %i'
        ...
        if cfg['mode'] == "Reduce":
            if 'data_ratio' in cfg.keys():
                cls_num_list = [int(len(l) * cfg['data_ratio']) for _, l in enumerate(cls_loc_list)]
            elif 'data_num' in cfg.keys():
                cls_num_list = [cfg['data_num'] // cls_num] * cls_num
            else:
                raise ValueError('Expect either "data_ratio" or "data_num" in config.')
        elif cfg['mode'] == "LT":
        ...
        for loc, num in zip(cls_loc_list, cls_num_list):
            if num <= len(loc):
                ...
                replicate_warning = True

        if replicate_warning:
            print('[warning] More sample required than available in this class, '
                  'samples are replicated and will introduce error in estimation')

        # print('Subset cls_num_list is: ' + str(cls_num_list))

        if not return_count:
            return subset_loc_list
        else:
            return subset_loc_list, list(cls_num_list)


# Long-Tailed Shift Test Set--------------------------------------------------#
def exp_imb_gen(img_max, cls_num, imb_factor):
    img_num_per_cls = []
    if img_max * imb_factor < 1:
        replicate_num = math.trunc(1 / (img_max * imb_factor)) + 1
        img_max *= replicate_num
        print('[warning] insufficient data to generate subset, replicate data by %i' % replicate_num)
...
def knockout_imb_gen(img_max, cls_num, ratio):
    ...
    img_num_per_cls = np.zeros(cls_num, dtype=int)
...
def tweakone_imb_gen(img_max, cls_num, rho, replace: bool = False):
    ...
    if rho < rho_min or rho > rho_max:
        print("Recommended range of rho is: " + str([rho_min, rho_max]))
...
def dirichlet_imb_gen(img_max, cls_num, alpha,
    ...
    if total_num is not None:
        ...
        if total_num > sample_num:
            print('[warning] Dirichlet shift config requires more sample number than available.')
