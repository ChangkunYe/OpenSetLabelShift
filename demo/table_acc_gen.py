...
def main():
    info = [
        {
            "csv_prefix": "/Users/changkunye/Projects/LOGS/OpenOOD/CIFAR10-20240807/",
            "dataset": 'cifar10',
        },
        {
            "csv_prefix": "/Users/changkunye/Projects/LOGS/OpenOOD/CIFAR100-20240807/",
            "dataset": 'cifar100',
        }
    ]
                        ...
                        for row in reader:
                            if j == data_row[0]:
                                if method != 'openmax':
                                    ...
                                    tab[count].append("$%s$" % (format_data(d0[0])))
                                    ...
                                    tab[count].append("$%s$" % (format_data(d[0])))
                                    ...
                                    # tab[count + 1].append("$%s_{\\scaleto{\\pm %s}{3pt}}$" %
                                    #                       (format_data(d0[0]), format_data(d0[-1])))
                                    tab[count + 1].append("$%s$" % (format_data(d0[0])))
                                ...
                                if method != 'openmax':
                                    ...
                                    # tab[count + 2].append("$%s_{\\scaleto{\\pm %s}{3pt}}$" %
                                    #                       (format_data(d0[0]), format_data(d0[-1])))
                                    tab[count + 2].append("$%s$" % (format_data(d0[0])))
                                    ...
                                    if round(float(d0[0]), prec) <= round(float(d1[0]), prec):
                                        ...
                                        #                       (format_data(d1[0]), format_data(d1[-1])))
                                        tab[count + 3].append("\\cc $%s$" % (format_data(d1[0])))
                                    else:
                                    ...
                                    # tab[count + 3].append("$%s_{\\scaleto{\\pm %s}{3pt}}$" %
                                    #                       (format_data(d[0]), format_data(d[-1])))
                                    tab[count + 3].append("$%s$" % (format_data(d[0])))
                                    ...
                                    #                       (format_data(d0[0]), format_data(d0[-1])))
                                    tab[count + 4].append("$%s$" % (format_data(d0[0])))
                                    ...
                                    if round(max(float(d0[0]), float(d[0])), prec) <= round(float(d1[0]), prec):
                                        ...
                                        #                       (format_data(d1[0]), format_data(d1[-1])))
                                        tab[count + 5].append("\\cc $%s$" % (format_data(d1[0])))
                                    else:
                                        # tab[count + 5].append("$%s_{\\scaleto{\\pm %s}{3pt}}$" %
                                        #                       (format_data(d1[0]), format_data(d1[-1])))
                                        tab[count + 5].append("$%s$" % (format_data(d1[0])))
