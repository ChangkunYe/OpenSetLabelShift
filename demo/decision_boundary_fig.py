...
def main():
    ...
    ax.fill_between(x3, y_max * np.ones_like(x3), color='b', alpha=0.1)
    ax.fill_between(x4, y_max * np.ones_like(x4), color='g', alpha=0.1)
    ax.fill_between(x, pi_1[0] * y1, color='lime', alpha=0.5, label=r"$Acc$")
    ax.fill_between(x, pi_1[1] * y2, color='lime', alpha=0.5)
    # ax.fill_between(x, pi_1[0] * y1, color='mediumslateblue', alpha=0.5, label=r"$Acc_1$")
    # ax.fill_between(x, pi_1[1] * y2, color='forestgreen', alpha=0.5, label=r"$Acc_2$")
    ax.fill_between(x, (pi_1[0] * y1 if wrong1 else pi_1[1] * y2) * overlap,
                    color='r', alpha=0.5, hatch='/', label=r"$1 - Acc$")
    ...
    ax.fill_between(x, pi_2[1] * y2, color='lime', alpha=0.5, label=r"$Acc$")
    ax.fill_between(x, (pi_2[0] * y1 if wrong1 else pi_2[1] * y2) * overlap,
                    color='r', alpha=0.5, hatch='/', label=r"$1 - Acc$")
    ...
    """
    ax2 = ax.twinx()
    # ax.set_title(r'Target Domain, $p_t(y=\cdot)=%s$' % pi_2)
    # ax.set_title(r'LS Corrected Mode')
    ax2.set_xlim(x_min, x_max)
    ax2.set_ylim(0, 1)
    ax2.set_ylabel(r"Confidence ($f(x)$)")
    ax2.plot(x, conf5, color='cyan', label=r"$\hat{f}(x)_1$", ls='-')
    ax2.plot(x, conf6, color='lime', label=r"$\hat{f}(x)_2$", ls='-')
    ax2.legend(loc='best')
    """
    ...
    ax.fill_between(x, pi_1[1] * y2, color='lime', alpha=0.5, label=r"$Acc$")
    ax.fill_between(x, (pi_1[0] * y1 if wrong2 else pi_1[1] * y2) * overlap2,
                    color='r', alpha=0.5, hatch='/', label=r"$1 - Acc$")
    # ax.axvline(boundary3, color='black', ls='--', label='Boundary')
    ax.legend(loc='upper right')
    ...
    ax.fill_between(x, pi_2[0] * y1, color='lime', alpha=0.5, label=r"$Acc_1$")
    ax.fill_between(x, pi_2[1] * y2, color='lime', alpha=0.5, label=r"$Acc_2$")
    ax.fill_between(x, (pi_2[0] * y1 if wrong2 else pi_2[1] * y2) * overlap2,
                    color='r', alpha=0.5, hatch='/', label=r"$1 - Acc$")
    # ax.axvline(boundary3, color='black', ls='--', label='Boundary')
    # ax.legend(loc='upper right')
