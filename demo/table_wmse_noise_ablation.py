...
headings = ['\\multicolumn{2}{c|}{\multirow{2}{*}{BBSE}}  & Near & ',
            ...
            '\\multirow{10}{*}{\\textbf{ours}} & \\multirow{2}{*}{$\gamma=0.1$}  &  Near &  ',
            '& & Far & ',
            ' & \\multirow{2}{*}{$\gamma=0.2$}  &  Near &  ',
            '& & Far & ',
            ' & \\multirow{2}{*}{$\gamma=0.3$}  &  Near &  ',
            '& & Far & ',
            '& \\multirow{2}{*}{$\gamma=0.4$}  &  Near &  ',
            '& & Far & ',
            ' & \\multirow{2}{*}{$\gamma=0.5$}  &  Near &  ',
            '& & Far & ',
            ]
...
def main():
    info = [
        {
            "csv_prefix": "/Users/changkunye/Projects/LOGS/OpenOOD/CIFAR100-20250127-noise_ablation/",
            "dataset": 'cifar100',
        }
    ]
    ...
    for data_info, name in zip(info_list, name_list):
        for id_shift in id_shifts:
            for ood_shift_type, ood_shift_name in zip(ood_shift_types, ood_shifts_names):
                write_path = "/Users/changkunye/Projects/LOGS/OpenOOD/all_tables/" \
                             "table_%s_wmse_%s-%s-%s-%s-noise_ablation.txt" % \
                             (method, name, estimate, id_shift, ood_shift_type)

                print_table(data_info, method, noise, ood_shift_name, ood_id_ratios, id_shift, write_path)
...
def main2():
    info = [
        {
            "csv_prefix": "/Users/changkunye/Projects/LOGS/OpenOOD/ImageNet200-20241112/",
            "dataset": 'imagenet200',
        },
    ]
...
def print_table(info, method, noise, ood_shifts, ood_id_ratios, id_shift, write_path,
                data_row=[3,8], all_rows=[19,24]):
    tab = []
    prec = 4
        ...
        for p in info:
            for o_shift in ood_shifts:
                for oi_ratio in ood_id_ratios:
                    file_stamp = "test_ood-%s-%s-%s-%s-%s-%s" % \
                                 (p['dataset'], id_shift, o_shift, oi_ratio, ni, method)
                    ...
                    with open(file_path) as f:
                        ...
                        for row in reader:
                            if j == data_row[0]:
                                ...
                                # tab[8].append("$%s$" % (format_data(d0[0])))
                                ...
                                for m,n in enumerate(range(all_rows[0] + 1, all_rows[1])):
                                    ...
                                    # tab[2 * m].append("$%s$" % (format_data(d[0])))
                                    if m == 0:
                                ...
                                if round(float(d0[0]), prec) > round(float(d1[0]), prec):
                                    ...
                                    if round(float(d1[0]), prec) < d_min:
                                        ...
                                        # tab[count + 1].append("\\cc $\\mathbf{%s}$" % (format_data(d1[0])))
                                    else:
                                        tab[count + 1].append("\\cc $%s_{\\scaleto{\\pm %s}{3pt}}$" %
                                                              (format_data(d1[0]), format_data(d1[-1])))
                                        # tab[count + 1].append("\\cc $%s$" % (format_data(d1[0])))
                                ...
                                for m,n in enumerate(range(all_rows[0] + 1, all_rows[1])):
                                    ...
                                    tab[2 * m + 1].append("$%s_{\\scaleto{\\pm %s}{3pt}}$" %
                                                       (format_data(d[0]), format_data(d[-1])))
                                    # tab[2 * m + 1].append("$%s$" % (format_data(d[0])))
                                    if m == 0:
                                        d_min = round(float(d[0]), prec)
                                    else:
                                        tmp = round(float(d[0]), prec)
                                ...
                                if round(float(d0[0]), prec) > round(float(d1[0]), prec):
                                    ...
                                    if round(float(d1[0]), prec) < d_min:
                                        ...
                                        tab[count + 2].append("\\cc $\\mathbf{%s}_{\\scaleto{\\pm %s}{3pt}}$" %
                                                               (format_data(d1[0]), format_data(d1[-1])))
                                        # tab[count + 2].append("\\cc $\\mathbf{%s}$" % (format_data(d1[0])))
                                    else:
                                        tab[count + 2].append("\\cc $%s_{\\scaleto{\\pm %s}{3pt}}$" %
                                                               (format_data(d1[0]), format_data(d1[-1])))
                                        #tab[count + 2].append("\\cc $%s$" % (format_data(d1[0])))
                                else:
                                    tab[count + 2].append("$%s_{\\scaleto{\\pm %s}{3pt}}$" %
                                                           (format_data(d1[0]), format_data(d1[-1])))
                                    # tab[count + 2].append("$%s$" % (format_data(d1[0])))
    ...
    with open(write_path, 'w') as f:
        f.write("\\multicolumn{12}{c}{Closed Set Label Shift estimation models} \\\\ \\hline\\hline \n")
        for i, line in enumerate(tab):
            print(len(line))
            if i == 8:
                f.write("\\hline \n")
                f.write("\\multicolumn{12}{c}{Open Set Label Shift estimation models} \\\\ \\hline\\hline \n")

            if (i < 8) and (i % 2 == 1):
                f.write(headings[i] + " & ".join(line) + "\\\\ \\hline \n")
            elif (8 < i < len(tab)) and (i % 2 == 0):
                f.write(headings[i] + " & ".join(line) + "\\\\ \\cline{2-12}\n")
            elif i == 8:
                f.write(headings[i] + " & ".join(line) + "\\\\ \\hline \n")
            else:
