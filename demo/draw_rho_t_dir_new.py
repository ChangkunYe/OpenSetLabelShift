...
shift_name = {
    ...
    'LT10r': "LT10 Backward",
    'LT50r': "LT50 Backward",
    'LT100r': "LT100 Backward",
    'Dir1': r"Dirichlet $\alpha=1$",
    'Dir10': r"Dirichlet $\alpha=10$",
}
...
def main():
    ...
    info = [
        {
            "csv_prefix": "/Users/changkunye/Projects/LOGS/OpenOOD/CIFAR10-20240816-dir/",
            "dataset": 'cifar10',
            "name": 'CIFAR10',
        },
        {
            "csv_prefix": "/Users/changkunye/Projects/LOGS/OpenOOD/CIFAR100-20240816-dir/",
            "dataset": 'cifar100',
            'name': 'CIFAR100',
        }
    ]
    ...
    # ood_shifts_names = [['LT10', 'LT10r'], ['LT50', 'LT50r'], ['LT100', 'LT100r']]
    ood_shifts_names = [['Dir1', 'Dir10']]

    # ood_shifts_names = [['LT10'], ['LT10r']]
    # ood_id_ratios = ["1.0", "0.1", "0.01"]
    ood_id_ratios = ["2.0", "1.0", "0.5", "0.2", "0.1", "0.01"]
    ...
    for ood_shift_name in ood_shifts_names:
        ...
        n_row, n_col = (plot_rows, 2)
        fig, axes = plt.subplots(n_row, n_col, sharey='row', figsize=(9.6, 4.0 * plot_rows))
        axes[0,0].set_ylim(0.2, 1)
        axes[0,0].set_ylabel(r'Predicted $\hat{\rho}_t$')
        axes[1,0].set_ylim(0.2, 1)
        axes[1,0].set_ylabel(r'Predicted $\hat{\rho}_t$')
            ...
            for o_shift in ood_shift_name:
                for p, name in zip(info, name_list):
                    ...
                    for method in methods:
                        ...
                        axes[i // 2, i % 2].set_xlabel(r'Ground truth $\rho_t$')
                        axes[i // 2, i % 2].plot(rho_t_gt, rho_t1_near, label=method_list[method] + r' $\hat{\rho}_t^*$',
                                     color=colors[k], linewidth=1.5)
                        axes[i // 2, i % 2].plot(rho_t_gt, rho_t1_near, 'o', color=colors[k])
                        axes[i // 2, i % 2].fill_between(rho_t_gt, np.array(rho_t1_near) - np.array(rho_t1_std_near),
                                             np.array(rho_t1_near) + np.array(rho_t1_std_near), alpha=0.2,
                                             color=colors[k])

                        axes[i // 2, i % 2].plot(rho_t_gt, rho_t0_near, '--', label=method_list[method] + r' $\hat{\rho}_t$',
                                     color=colors[k], linewidth=1.5)
                        axes[i // 2, i % 2].plot(rho_t_gt, rho_t0_near, '*', color=colors[k])
                        axes[i // 2, i % 2].fill_between(rho_t_gt, np.array(rho_t0_near) - np.array(rho_t0_std_near),
                                             np.array(rho_t0_near) + np.array(rho_t0_std_near), alpha=0.2,
                                             color=colors[k])


                    axes[i // 2, i % 2].set_title("%s, %s" % (p['name'], shift_name[o_shift]))
                    axes[i // 2, i % 2].plot(rho_t_gt, rho_t_gt, 'k', label='Ground Truth', linewidth=2.0)
                    # axes[i].plot(rho_t_gt, np.ones_like(rho_t_gt) / 2, 'r', label='Baseline', linewidth=1.5)
             # plt.legend(loc='best')

    handles, labels = axes[0,0].get_legend_handles_labels()
    lgd = fig.legend(handles, labels, bbox_to_anchor=(0.9, 0.00), ncol=3)
    plt.tight_layout()
    plt.savefig('./rho_t-cifar-dir2.pdf', bbox_extra_artists=(lgd,), bbox_inches='tight')
    plt.show()
    plt.close()
...
def main2():
    ...
    for ood_shift_name in ood_shifts_names:
        ...
        plt.rcParams.update(params)
        n_row, n_col = (plot_rows, 2)
        fig, axes = plt.subplots(n_row, n_col, sharey='row', figsize=(9.6, 4 * plot_rows))
            ...
            for o_shift in ood_shift_name:
                for p, name in zip(info, name_list):
                    ...
                    for method in methods:
                        ...
                        axes[i // 2, i % 2].set_xlabel(r'Ground truth $\rho_t$')
                        axes[i // 2, i % 2].plot(rho_t_gt, rho_t1_near, label=method_list[method] + r' $\hat{\rho}_t^*$')
                        axes[i // 2, i % 2].plot(rho_t_gt, rho_t1_near, "o")
                        axes[i // 2, i % 2].fill_between(rho_t_gt, np.array(rho_t1_near) - np.array(rho_t1_std_near),
                                         np.array(rho_t1_near) + np.array(rho_t1_std_near), alpha=0.2)

                        axes[i // 2, i % 2].plot(rho_t_gt, rho_t0_near, '--', label=method_list[method] + r' $\hat{\rho}_t$')
                        axes[i // 2, i % 2].plot(rho_t_gt, rho_t0_near, 'o')
                        axes[i // 2, i % 2].fill_between(rho_t_gt, np.array(rho_t0_near) - np.array(rho_t0_std_near),
                                         np.array(rho_t0_near) + np.array(rho_t0_std_near), alpha=0.2)

                    axes[i // 2, i % 2].set_title("%s, %s" % (p['name'], shift_name[o_shift]))
                    axes[i // 2, i % 2].plot(rho_t_gt, rho_t_gt, 'k', label='GT', linewidth=2.0)
                    # axes[i].plot(rho_t_gt, np.ones_like(rho_t_gt) / 2, 'r', label='Baseline', linewidth=1.5)
             # plt.legend(loc='best')

        handles, labels = axes[0,0].get_legend_handles_labels()
        lgd = fig.legend(handles, labels, bbox_to_anchor=(0.9, 0.0), ncol=3)
        plt.tight_layout()
        plt.savefig('./rho_t-imagenet200.pdf', bbox_extra_artists=(lgd,), bbox_inches='tight')
        plt.show()
        plt.close()
